...
\begin{align}
\vec{x}^T\vec{x}-2\vec{O}^T\vec{x}+\| O\|^2-\vec{r}^2=0\\
\end{align}
where O is the origin and r is the radius of the circle\\
we get,
\begin{align}
\vec{O}=\myvec{O_x\\O_y}\\
...
\vec{m}=\myvec{1\\0}
\end{align}
For the given circle the tangent parallel to x-axis with normal "{n}" is
\begin{align}
\vec{P_i}=(\vec{k_i}\vec{n}-\vec{u})
\end{align}
where i=1,2,3.....
\begin{align}
\vec{k_i}=\pm\sqrt{\frac{\vec{u}^T\vec{u}-\vec{f}}{\vec{n}^T\vec{n}}}\\
\vec{k_i}=\pm2
\end{align}
The point on the circle where tangent is parallel to x-axis is given by
\begin{align}
\vec{k_i}=2; \vec{P_1}=\myvec{1\\2}\\
\vec{k_i}=-2; \vec{P_2}=\myvec{1\\-2}
\end{align}
